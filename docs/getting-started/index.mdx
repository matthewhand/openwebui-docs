...
If you want to disable login for a single-user setup, set the environment variable [`WEBUI_AUTH`](/getting-started/env-configuration) to `False`. This will bypass the login page.
...
- Use the **Ollama API**: Check the model list with:
  ...
  ```
Explore more in our [Next Steps](/getting-started/next-steps) guide.
---
...
Refer to the full [Development Guide](/getting-started/development) for detailed steps.
...
kubectl apply -k ./k8s-manifests
```
Ensure your Kubernetes cluster is correctly configured and refer to our [Kubernetes Documentation](/getting-started/kubernetes) for troubleshooting.
...
Thank you for choosing Open WebUI! We are committed to providing a powerful, privacy-focused interface for your LLM needs. If you encounter any issues, refer to the [Troubleshooting Guide](/getting-started/troubleshooting).
