...
```bash
sed -i "s/127.0.0.1:8080/0.0.0.0:1337/" docker-compose.yaml
```
...
6. Delete the default `settings.yml` file, it will be regenerated on first launch:
...
```bash
docker compose up searxng -d ; sleep 10 ; docker compose down searxng
```

8. Swap the HTML output for JSON:

```bash
sed -i 's/- html/- json/' searxng-docker/settings.yml
```

#### Configuration Files

8. Your `searxng-docker/settings.yml` file for SearXNG should look like:
...
```bash
docker run -d --name searxng -p 1337:1337 -v ./searxng:/etc/searxng --restart always searxng/searxng:latest
```
